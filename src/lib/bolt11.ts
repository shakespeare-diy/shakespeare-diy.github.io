 ...
 * commits to. `nip57.getSatoshisAmountFromBolt11` reads the human-readable
...
export function invoiceCommitsTo(decoded: DecodedInvoice, candidates: string[]): boolean {
  ...
  // Use noble's own `utf8ToBytes` rather than `TextEncoder`: under jsdom the
  // latter returns a Uint8Array from a different realm, which `sha256` rejects.
